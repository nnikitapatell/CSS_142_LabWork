...
public class TODO3 extends SpaceSmasherFunctionalAPI {
    ...
    //define this function to work on moving paddles in response to user input 
    public void paddleMovementCheck() {   
        if (isKeyboardButtonDown(LEFT)){
            paddleMoveLeft();   //moves the paddle left one unit
        }
        if (isKeyboardButtonDown(LEFT)){
            paddleMoveRight();  //moves the paddle right one unit
        }
}
}        
